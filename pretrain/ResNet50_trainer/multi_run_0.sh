...
<<!
--category_num=1000 \
                          --per_device_train_batch_size=80 --weight_decay=0.0001 --label_smoothing=0.1 --train_num=1300 \
                          --test_num=100 --learning_rate=0.05 --minimum_learning_rate=0.0001 \
                          --save_steps=20  --num_train_epochs=2 --warmup_steps=5 \
                          --do_predict=True --per_device_eval_batch_size=10 --gradient_accumulation_steps=10 \
                          --adam_beta1=10 --adam_epsilon=10 --max_grad_norm=1.0 --max_steps=5 --warmup_ratio=1 \
                          --logging_steps=10 --fp16=False  --model_name_or_path=empty --train_list_path=data/list_train.txt --test_list_path=data/validation_test.txt
!