...
class ViT(tf.keras.Model):
    ...
    def __init__(self, *, image_size, patch_size, num_classes, dim, depth, heads, mlp_dim, hidden_layer_shape, channels=3):
        super().__init__()
        ...
        self.transformer = Transformer(dim, depth, heads, hidden_layer_shape)
        self.to_cls_token = tf.identity

        self.mlp_head = tf.keras.Sequential([tf.keras.layers.Dense(mlp_dim, kernel_initializer="he_normal", activation=get_activation('gelu')),
                                        tf.keras.layers.Dense(num_classes, name='fully_connected', kernel_initializer="he_normal", activation='softmax', use_bias=False)])
