...
def train_step(dist_inputs):
    ...
    per_replica_ce = mirrored_strategy.run(step_fn, args=(dist_inputs,))
...
# First, we create the model and optimizer inside the strategy's scope. This ensures that any variables created with the model and optimizer are mirrored variables.
    ...
    with mirrored_strategy.scope():
        ...
        learning_rate_schedules = CosineDecayWithWarmUP(initial_learning_rate=c.initial_learning_rate * 2,
...
# Next, we create the input dataset and call `tf.distribute.Strategy.experimental_distribute_dataset` to distribute the dataset based on the strategy.
    ...
    #  eval
    dataset_eval = test_iterator().batch(global_batch_size, drop_remainder=True)
    ...
    #     sum_ce = 0
    #     sum_correct_num = 0
    #     for i in tqdm(range(c.test_iterations // num_gpus)):
    #         ce, correct_num = test_step(next(test_data_iterator))
    #         # print(loss_all)
    #         sum_ce += ce 
    ...
    #         f.write('epoch:{}\n'.format(epoch_num))
            ...
            # train
        sum_ce = 0
        for i in tqdm(range(int(c.iterations_per_epoch/num_gpus))):
            loss_ce = train_step(next(iter(train_data_iterator)))
            # print(loss_all)
            sum_ce += loss_ce 
            print('train batch ce loss: {:.4f}'.format(loss_ce))    
            # f.write('train: cross entropy loss: {:.4f}\n'.format(sum_ce / c.train_num))


        sum_ce = 0
        sum_correct_num = 0
        for i in tqdm(range(int(c.test_iterations / num_gpus))):
            ce, correct_num = test_step(next(iter(test_data_iterator)))
            # print(loss_all)
            sum_ce += ce 
            sum_correct_num +=  correct_num
