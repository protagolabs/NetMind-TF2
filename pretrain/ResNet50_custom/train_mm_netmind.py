...
if __name__ == '__main__':
    ...
    with mirrored_strategy.scope():
        ...
        learning_rate_schedules = CosineDecayWithWarmUP(initial_learning_rate=c.initial_learning_rate * num_gpus,
                                                        decay_steps=c.epoch_num * int(
                                                            c.iterations_per_epoch / num_gpus) - int(
                                                            c.warm_iterations / num_gpus),
    ...
    with open(c.log_file, 'a') as f:
        ...
        for epoch_num in range(epochs_trained, c.epoch_num):
            ...
            if epoch_num % 5 == 4:
                os.system(
                    'cp {} {}_epoch_{}.h5'.format(c.save_weight_file, c.save_weight_file.split('.')[0], epoch_num))
